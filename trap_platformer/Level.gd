...
func _draw():
    for p in platforms:
        ...
        draw_line(p.position, p.position + Vector2(p.size.x, 0), Color("#8c9aa8"), 3)
    ...
    # Exit
    draw_rect(Rect2(875, 255, 35, 45), Color("#7ee787"))
    draw_string(ThemeDB.fallback_font, Vector2(820, 235),
        "EXIT", HORIZONTAL_ALIGNMENT_LEFT, -1, 18, Color("#7ee787"))
